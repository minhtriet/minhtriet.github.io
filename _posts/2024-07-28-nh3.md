...
BO, being a gradient-less method, does not have these requirements. Here is how it works. After the initial samplings, Gaussian processing (GP) fixes a regression line through these data points. Only the observed datapoints $$\pmb{X}=x_1, x_2, ... , x_n$$ has absolute certainty (variance = 0). For an unknown point $$x'$$, GP returns a prediction from the distribution of $$\mathscr{N}(\mu,\Sigma\lvert\pmb{X})$$. 
	...
	<div class="column">
 	<figure>
		<img src="https://github.com/user-attachments/assets/af199fd0-5dbe-4a2c-addb-e9fb6f2ecd2b">
  		<figcaption>Given initial samplings (red points), how do we predict an unknown point?</figcaption>
  	</figure>
	</div>
	<div class="column">
 	<figure>
		<img src="https://github.com/user-attachments/assets/033cceb6-5ea3-4663-af1f-b17e4708a085">
  		<figcaption>How GP fit given these groundtruth. The confidence interval is the variances of the Gaussian at an arbitrary point, and the prediction is the means</figcaption>
  	</figure>
	</div>
</div>
...
Then we use the acquision function $$\alpha$$ to optain new positions and pass then to the Hamilton, keeping log of them and estimate the posterior value of the ground state enerygy given the coordinates. There are several approaches to define $$\alpha$$, we are using Expected Improvement approach here.
