...
$$\text{NH}_3$$ production uses 1% of the world energy. The most widely adopted synthesis method, Haber-Bosch process, is
...
BO, being a gradient-less method, does not have these requirements. Here is how it works. After the initial samplings $$\pmb{X}=x_1, x_2, ... , x_n$$ , Gaussian processing (GP) assume that $$\pmb{X} \sim \mathscr{N}\begin{bmatrix}\begin{pmatrix}0\\0\\0\end{pmatrix},\begin{pmatrix}3 & d & d\\d & 1 & d\\d & d & 1\end{pmatrix}
\end{bmatrix}$$ and fixes a regression curve through these data points. Only the observed datapoints has absolute certainty (variance = 0). For an unknown point $$x'$$, GP returns a prediction from the distribution of $$\mathscr{N}(\mu,\Sigma\lvert\pmb{X})$$.
