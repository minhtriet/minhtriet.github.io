...
$$\pmb{X} \sim \mathscr{N}\begin{bmatrix}\begin{pmatrix} \mu_1 \\ \cdot \\ \mu_n\end{pmatrix},\begin{pmatrix}1 & K_{12} & \cdots & K_{1n}\\K_{21} & \vdots & \ddots & \vdots \\ K_{n1} & K_{n2} & \cdots & K_{nn} \end{pmatrix} \end{bmatrix}$$ 

and fixes a regression curve through these data points. Only the observed datapoints has absolute certainty (variance = 0). For an unknown point $$x'$$, GP returns a prediction from the distribution of $$\mathscr{N}(\mu,\Sigma\lvert\pmb{X})$$.
