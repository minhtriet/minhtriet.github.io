...
# Method
There are two methods in use, gradient descent and Bayesian optimization. The former method has good coverage in these [Pennylane](https://pennylane.ai/qml/demos/tutorial_chemical_reactions/) [demoes](https://pennylane.ai/qml/demos/tutorial_mol_geo_opt/). Here we focus on the latter.

## Motivation
To define a affine transformation for a molecule, we need to define translation parameter $$t_x, t_y, t_z$$ and rotational parameter $$\theta_x,\theta_y,\theta_z$$. When calculating the gradientfor gradient descent, we need to know $$t_\bullet \pm \Delta_t$$ and $$\theta_\bullet \pm \Delta_\theta$$. Calculating for the Hamiltonian is expensive. For each ingredient we needs to calculate $6 \times 2$ Hamiltonians before each coordinate update.
...
Acquisition function $$\alpha$$, given the $$H$$ and $$X$$, what is the next point we want to evaluate. It is based on the confidence interval
...
<div class="row">
	<div class="column">
		<figure>
	<img alt="image" width="200px" src="https://github.com/user-attachments/assets/c981aea2-1fb2-4cad-95b8-0ba84b8cee11">
	...
	<div class="column">
		<figure>
	<img width="200px" src="https://github.com/user-attachments/assets/aa6a75ec-c8f5-406b-82df-9de6bb4838a6">
...
<span class="boxed h">
      $$H$$
</span>. Please open the GIFs in new tab for a full resolution version.
...
<figure>
 ...
 <div class="row">
<div class="column">
![output-onlinegiftools](https://github.com/user-attachments/assets/a6aabde4-23e7-4030-a508-5fffa8d62f2c)
</div>
<div class="column">
</div>
	![god](https://github.com/user-attachments/assets/00a11c8c-ebe6-4199-b38a-ea71e6d24487)

 </div>
Note that there are times that the BO choose to sample two very close points, it is because of the exploitation
## Recreating the change in energy
