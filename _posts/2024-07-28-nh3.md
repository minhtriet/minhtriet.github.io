...
$$\text{NH}_3$$ molecule is produced in a total of 6 frames (namely 12th, 13th, 25th, 26th, 27th, 28th), that matches the efficiency of the Harber-Bosch process of about 20%[4]. It is because the coordinates that create $$text{NH}_3$$ still have higher ground state energy in comparison with the config where $$\text{H}$$ atoms  float away. Note that there are times that the BO choose to sample two very close points, it is because of the exploitation and exploration trade-of that we mentioned earlier
