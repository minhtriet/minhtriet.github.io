...
In this post, we run on the first three steps in Fugure 2, which is the most complex reactions. Through this, we contribute an approach to simulate chemistry reaction with (1) catalyst and (2) endothermicity

# Method
The gradient descent method for geometric optimization has good coverage in [Pennylane demo](). Here we focus on the Bayesian method
