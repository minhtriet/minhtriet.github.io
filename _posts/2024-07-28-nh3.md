...
A beautiful propertiy of Guassian is that when conditioning the Gaussian on a set of variables, the result is also a Gaussian $$\mathscr{N}(\mu',\Sigma')$$. In Fig.2, the prediction value of unknown point which is the mean value Gaussian, whereas the interval confidence is its variant or confidence interval. 
<figure>
<div class="row">	
	<div class="column">
 	<figure>
...
<figcaption>Figure 2: A. Given initial samplings (red points), how do we predict an unknown point? B. How GP fit given these groundtruth. The confidence interval is the variances of the Gaussian at an arbitrary point, and the prediction is the means</figcaption>
</figure>

Here we faces a dilemma. In $$\pmb{X}$$ there is a point with the smallest value $$x_\min$$. Therefore, the next minima should be somewhere close to it. However, it is also possible that points in the wide confidence interval have smaller values than the current minima. To quantify this trade-off, we define an acquision function $$\alpha$$ to optain new positions to sample. There are several approaches to define $$\alpha$$, we are using Expected Improvement (EI) approach here.

EI works by generating random functions that lie inside the confident intervals, as demonstrated in Figure 3. The intuition is a larger confidence interval would have more diverse sampling functions. Then we would sample the point at the extrema and update the prior. The process continue for a predefined number of steps.
<div class="row">
	<div class="column">
<figure>
<img src="https://github.com/user-attachments/assets/a3cd9cbd-da05-40a2-a61d-8f18760cda38">
<figcaption>Figure 3. Given the prior in solid blue, EI generate two candidates (dashed red and green lines) that satisfies the confidence interval requirements. Then we sample the real value at the point marked with "?"</figcaption>
</figure>
	</div>
</div>
...
# Visualization
We are going to replicate the first three steps of Figure 2, whose Hamiltonian are the most complex in the whole pathway. 
...
We figured out that most of the  $$\text{H}$$ went too far away from the catalyst substrate, due to our initial set up of the search margin, which is $$1 nm$$ bigger than the catalyst substrate. Here we filter only what went inside the substrate.
...
$$\text{NH}_3$$ molecule is produced in a total of 6 trials (namely 12th, 13th, 25th, 26th, 27th, 28th), that matches the efficiency of the Harber-Bosch process of about 20%[4]. We can also see that the coordinates that create $$text{NH}_3$$ still have higher ground state energy in comparison with the config where $$\text{H}$$ atoms float away. Note that there are times that the BO choose to sample two very close points cosecutively (12th and 13th frame for example), it is because of the exploitation and exploration trade-of that we mentioned earlier
...
1. Due to the lack of computational resource we have to reduce the catalyst platform, number of active orbitals and electrons. Even with a simple orbital basis and reduced numbers, it still take considerable amount of time for the experiments
	- Gradient descent: 32 CPU, 256 GB RAM, 50h run
	- BO: 8 CPU, 64 GB RAM, 60h run
3. The number of active electrons and oribtals. These parameters are visible in `chem_config.yaml`
