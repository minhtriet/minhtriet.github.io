...
<figure>
  ...
  <figcaption>Fig 1. Reaction pathway of the Harber-Bosch process in Reiher et al.[^1] </figcaption>
</figure>
...
$$\text{NH}_3$$ molecule is produced in a total of 6 trials (namely 12th, 13th, 25th, 26th, 27th, and 28th), which coincidentally matches the efficiency of the Harber-Bosch process of about 20%[^a]. It also offers an explnation for that number, which is the coordinates that create $$\text{NH}_3$$ have higher ground state energy than the other configurations where $$\text{H}$$ atoms float away. Note that there are times that the BO chooses to sample two very close points consecutively (12th and 13th frame for example), it is because of the exploitation and exploration trade-of that we mentioned earlier
