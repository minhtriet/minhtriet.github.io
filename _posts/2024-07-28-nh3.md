...
# Introduction
The Haber-Bosch process enables the large-scale synthesis of amonia . Its reaction is: $$\text{N}_2 + 3\text{H}_2 \overset{Fe}{\longrightarrow} 2\text{NH}_3$$. This process, however, is energy-intensive and relies heavily on iron-based catalysts to lower the activation energy and increasing the reaction rate. Understand the reaction pathway with catalyst hopes to gain insights into the process, thus enhance its efficiency. 

Traditionally, this optimization is performed using Density Functional Theory (DFT), but quantum computing offers an alternative. We want to
# Contribution
<figure>
  <img width="1024" alt="image" src="https://github.com/user-attachments/assets/00af4999-f427-478d-bf28-cd2eea8dc342">
  <figcaption>Fig 1. A common work flow to tackle the problem with computers from [2]. </figcaption>
</figure>
The above figure shows that most of the steps are ran in a classical computer and the quantum computer only calculate the eigenvalues of a given Hamiltonian. This post explores how quantum computing can be used to optimize the geometry of a Haber-Bosch process using the Fe 211 catalyst.
