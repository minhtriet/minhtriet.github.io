...
<div class="row">
	...
	<figcaption>A. Ground truth from [1] B. Our simulation from a randomized position C. The ground state energy of the Hamiltonian</figcaption>
</figure>
...
$$\text{NH}_3$$ molecule is produced in a total of 6 trials (namely 12th, 13th, 25th, 26th, 27th, and 28th), which matches the efficiency of the Harber-Bosch process of about 20%[4]. We can also see that the coordinates that create $$\text{NH}_3$$ still have higher ground state energy than the config where $$\text{H}$$ atoms float away. Note that there are times that the BO chooses to sample two very close points consecutively (12th and 13th frame for example), it is because of the exploitation and exploration trade-of that we mentioned earlier
